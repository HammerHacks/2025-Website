...
export function useDecodeText(
  ...
  const currentTextsRef = useRef<string[]>(initialValues);
  ...
  const triggerDecode = useCallback(
    (newValues: string[]) => {
      ...
      const oldTexts = [...currentTextsRef.current];
      ...
      function update(now: number) {
        ...
        currentTextsRef.current = nextDisplay;
        setDisplayTexts(nextDisplay);

        if (progress < 1) {
          animationRef.current = requestAnimationFrame(update);
        } else {
          currentTextsRef.current = newValues;
